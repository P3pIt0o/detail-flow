...
export function StepVehicles({
  ...
  function addVehicle() {
    onChange([...vehicles, { uid: crypto.randomUUID(), serviceId: null, vehicleTypeId: null, optionIds: [] }])
  }
  ...
  return (
    <div className="space-y-6">
      {vehicles.map((v, index) => {
        const preview =
          v.serviceId && v.vehicleTypeId ? resolvePrice(services, priceMap, v.serviceId, v.vehicleTypeId) : null
        return (
          <div key={v.uid} className="rounded-xl border border-border bg-card p-5">
            ...
            <fieldset className="mb-5">
              <legend className="mb-2 text-sm font-medium text-card-foreground">Prestation</legend>
            ...
            {/* Type de véhicule */}
            <fieldset className="mb-5">
              <legend className="mb-2 text-sm font-medium text-card-foreground">Type de véhicule</legend>
            ...
            {options.length > 0 && (
              <fieldset>
                <legend className="mb-2 text-sm font-medium text-card-foreground">
                  Options complémentaires <span className="text-muted-foreground">(facultatif)</span>
                </legend>
                <div className="grid gap-2 sm:grid-cols-2">
                  {options.map((o) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="shrink-0 text-sm text-muted-foreground">+{formatPrice(o.priceCents)}</span>
                        <input type="checkbox" className="sr-only" checked={checked} onChange={() => toggleOption(v.uid, o.id)} />
                      </label>
                    )
                  })}
                </div>
            ...
            {preview && (
              <p className="mt-4 text-sm text-muted-foreground">
                Sous-total véhicule : <span className="font-semibold text-primary">{formatPrice(preview.priceCents + v.optionIds.reduce((s, id) => s + (options.find((o) => o.id === id)?.priceCents ?? 0), 0))}</span>
              </p>
            )}
          </div>
        )
