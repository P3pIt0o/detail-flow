...
const TRAVEL_ERRORS: Record<string, string> = {
  address_not_found: "Adresse introuvable. Vérifiez l'orthographe et ajoutez le code postal.",
...
export function StepContact({ contact, onChange, travel, onTravel, roundTrip, freeDistanceKm }: Props) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="font-serif text-base font-semibold text-card-foreground">Vos coordonnées</h3>
      ...
      {/* Adresse & déplacement */}
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="font-serif text-base font-semibold text-card-foreground">Adresse d'intervention</h3>
        <div className="mt-4 space-y-3">
          ...
          >
            {calculating ? <Loader2 className="h-4 w-4 animate-spin" /> : <MapPin className="h-4 w-4" />}
          ...
          {travel && !travel.ok && (
            <div className="flex items-start gap-2 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">
              ...
              <span>{travel.error ? TRAVEL_ERRORS[travel.error] : "Calcul impossible."}</span>
            </div>
          )}
            ...
            <div className="rounded-lg border border-border bg-background p-3 text-sm">
              ...
              <dl className="space-y-1 text-muted-foreground">
                <div className="flex justify-between">
                  ...
                  <dd className="text-card-foreground">{formatKm(travel.distanceKm)}</dd>
                </div>
                <div className="flex justify-between">
                  <dt>Distance gratuite</dt>
                  <dd className="text-card-foreground">{formatKm(freeDistanceKm)}</dd>
                </div>
                <div className="flex justify-between">
                  <dt>Distance facturée {roundTrip ? "(aller-retour)" : ""}</dt>
                  <dd className="text-card-foreground">{formatKm(travel.billedDistanceKm)}</dd>
                </div>
                <div className="flex justify-between border-t border-border pt-1 font-medium">
                  <dt className="text-card-foreground">Frais de déplacement</dt>
                  <dd className="text-primary">{travel.feeCents === 0 ? "Offert" : formatPrice(travel.feeCents)}</dd>
                </div>
              </dl>
            </div>
          )}
